    ...
    async executeSignal(signal: ParsedSignal, userId: string, sourceChatId?: string): Promise<TradeResult | undefined> {
        try {
            ...
            if (signal.type === 'TRADE') {
                ...
                // For LIMIT orders: PositionMonitor handles them after the fill is detected.
                const shouldPlaceExtraTps = (resolvedOrderType === 'market' || isExplicitlyFilled);
                ...
                if (shouldPlaceExtraTps) {
                    ...
                    logger.info(`⏳ [Limit Order] Extra TPs (TP2+) will be placed after order ${order.id} is filled. Attached SL/TP1 are already active.`);
                }
